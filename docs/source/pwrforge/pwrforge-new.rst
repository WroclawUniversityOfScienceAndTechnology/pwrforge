...
::
    ...
    --ci [gitlab|github]           [default: gitlab]

Choose CI/CD files generated by ``pwrforge update`` for the new project.
GitLab generates ``.gitlab-ci.yml`` and ``.devcontainer/.gitlab-ci-custom.yml``.
GitHub generates reusable workflows under ``.github/workflows``.
