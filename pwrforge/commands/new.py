...
def process_chips(chips: List[str], targets: List[pwrforgeTarget]) -> Dict[str, str]:
    targets_chips = {}
    unsupported_chips = []
...
def pwrforge_new(
    ...
    targets.extend([pwrforgeTarget(key) for key in targets_chips if key not in targets])
    if len(targets) == 0:
    ...
    config = get_pwrforge_config_or_exit(toml_path)
    generate_cpp(config)
