...
class _pwrforgeFlash:
    ...
    def _validate_erase_memory(self) -> None:
        if self._erase_memory and self._target.id != pwrforgeTarget.stm32:
    ...
    def _check_bin_path(self, bin_path: Path) -> None:
        if not bin_path.is_file():
            ...
            logger.info(
                "Did you run pwrforge build --profile %s --target %s",
                self._flash_profile,
                self._target.id,
            )
            sys.exit(1)
    ...
    def flash_target(self) -> None:
        if self._target.id == pwrforgeTarget.atsam:
            self._flash_atsam()
        elif self._target.id == pwrforgeTarget.esp32:
            self._flash_esp32()
        elif self._target.id == pwrforgeTarget.stm32:
            self._flash_stm32()
...
def pwrforge_flash(
    ...
    flasher = _pwrforgeFlash(flash_profile, port, target, app, file_system, erase_memory, bank)
