...
def mock_prepare_config(tmpdir: Path, mocker: MockerFixture) -> MagicMock:
    ...
    return mocker.patch(f"{pwrforge_gen.__module__}.prepare_config", return_value=x86_config)
...
def mock_prepare_config_esp32(tmpdir: Path, mocker: MockerFixture) -> MagicMock:
    ...
    return mocker.patch(f"{pwrforge_gen.__module__}.prepare_config", return_value=esp_config)
...
def test_gen_ut(mock_prepare_config: MagicMock, mocker: MockerFixture) -> None:
    ...
    mocked_generate_ut = mocker.patch(f"{pwrforge_gen.__module__}.generate_ut")
...
def test_gen_mock(
    ...
    mocked_generate_ut = mocker.patch(f"{pwrforge_gen.__module__}.generate_mocks")
...
def test_gen_mock_fails(
    ...
    mocked_generate_ut = mocker.patch(f"{pwrforge_gen.__module__}.generate_mocks", return_value=False)
    path_to_hdr = Path("header.h")
...
def test_gen_certs(
    ...
    pwrforge_root = Path(__file__).parent.parent.parent
    script_path = pwrforge_root / "pwrforge/certs/generateAllCertificates.sh"
