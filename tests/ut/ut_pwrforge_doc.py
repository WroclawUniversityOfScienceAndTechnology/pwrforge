...
def mock_prepare_config(mocker: MockerFixture, config: Config) -> MagicMock:
    return mocker.patch(f"{pwrforge_doc.__module__}.prepare_config", return_value=config)


@pytest.fixture
def mock_find_program_path(mocker: MockerFixture) -> MagicMock:
    return mocker.patch(f"{pwrforge_doc.__module__}.find_program_path", return_value="doxygen")


@pytest.fixture
def mock_typer_launch(mocker: MockerFixture) -> MagicMock:
    return mocker.patch(f"{pwrforge_doc.__module__}.typer.launch")
