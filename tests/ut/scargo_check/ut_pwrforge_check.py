...
from pwrforge.commands import check
...
def mock_checkers(mocker: MockerFixture) -> Dict[str, MagicMock]:
    checkers = {
        checker_class.check_name: mocker.patch(f"{pwrforge_check.__module__}.{checker_class.__name__}")
...
def mock_prepare_config(mocker: MockerFixture, config: Config) -> MagicMock:
    return mocker.patch(f"{pwrforge_check.__module__}.prepare_config", return_value=config)


def test_pwrforge_check_single(
...
def test_pwrforge_check_default(
    mock_checkers: Dict[str, MagicMock],
    mock_prepare_config: MagicMock,
) -> None:
    pwrforge_check(
