...
from pwrforge.config import Config, Target
...
def test_pwrforge_build_dir_exist(
    ...
    pwrforge_build(profile, None)
    assert build_dir.is_dir()


def test_pwrforge_build_no_cmake(
    fp: FakeProcess,
    fs: FakeFilesystem,
    ...
    assert ("INFO", "Did you run `pwrforge update`?") in log_data


def test_pwrforge_build_fails(
    fp: FakeProcess,
    fs: FakeFilesystem,
    mock_prepare_config: MagicMock,
    caplog: pytest.LogCaptureFixture,
    ...
    with pytest.raises(SystemExit):
        pwrforge_build(profile, None)
    log_data = get_log_data(caplog.records)
    assert ("ERROR", "pwrforge build target x86 failed") in log_data


def test_pwrforge_build_all_targets(
    fp: FakeProcess,
    ...
    pwrforge_build(profile, None, all_targets=True)
...
def mock_prepare_multitarget_config(tmpdir: Path, mocker: MockerFixture) -> MagicMock:
    ...
    return mocker.patch(f"{pwrforge_build.__module__}.prepare_config", return_value=multitarget_config)


@pytest.fixture
def mock_prepare_config(mocker: MockerFixture, config: Config) -> MagicMock:
    return mocker.patch(f"{pwrforge_build.__module__}.prepare_config", return_value=config)
