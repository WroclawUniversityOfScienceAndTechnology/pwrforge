...
from pwrforge.commands.debug import EMEDDED_GDB_SETTINGS, pwrforge_debug
...
def mock_debug_config(request: pytest.FixtureRequest, tmpdir: Path, mocker: MockerFixture) -> MagicMock:
    ...
    mocker.patch(f"{pwrforge_debug.__module__}.sleep")
    ...
    return mocker.patch(f"{pwrforge_debug.__module__}.prepare_config", return_value=test_config)
