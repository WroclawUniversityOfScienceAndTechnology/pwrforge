...
def test_pwrforge_test_no_test_dir(
    config: Config,
    fs: FakeFilesystem,
...
def test_pwrforge_test_no_cmake_file(
    config: Config,
    caplog: pytest.LogCaptureFixture,
    mock_prepare_config: MagicMock,
    fs: FakeFilesystem,
...
def test_pwrforge_test(config: Config, fp: FakeProcess, fs: FakeFilesystem, mock_prepare_config: MagicMock) -> None:
    config.project_root = Path(".")
    subprocess_commands = [
        ["conan", "profile", "list"],
...
def mock_prepare_config(mocker: MockerFixture, config: Config) -> MagicMock:
    return mocker.patch(f"{pwrforge_test.__module__}.prepare_config", return_value=config)
