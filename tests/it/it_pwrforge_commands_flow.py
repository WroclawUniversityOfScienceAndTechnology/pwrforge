...
class ActiveTestState:
    ...
    def assert_build_result_files_paths(self, profile: str = "Debug") -> None:
        ...
        if self.target_id in (pwrforgeTarget.stm32, pwrforgeTarget.atsam):
...
def active_state_stm32_bin() -> ActiveTestState:
    return ActiveTestState(
        target_id=pwrforgeTarget.stm32,
...
def active_state_esp32_bin() -> ActiveTestState:
    return ActiveTestState(
        target_id=pwrforgeTarget.esp32,
...
def active_state_atsam_bin() -> ActiveTestState:
    return ActiveTestState(
        target_id=pwrforgeTarget.atsam,
...
def active_state_x86_path() -> ActiveTestState:
    project_path = TEST_DATA_PATH / "test_projects/common_pwrforge_project"
    return ActiveTestState(
        target_id=pwrforgeTarget.x86,
...
def active_state_esp32_path() -> ActiveTestState:
    project_path = TEST_DATA_PATH / "test_projects/common_pwrforge_project_esp32"
    return ActiveTestState(
        target_id=pwrforgeTarget.esp32,
        proj_name=project_path.name,
        proj_to_copy_path=project_path,
...
def active_state_stm32_path() -> ActiveTestState:
    project_path = TEST_DATA_PATH / "test_projects/common_pwrforge_project_stm32"
    return ActiveTestState(
        target_id=pwrforgeTarget.stm32,
        proj_name=project_path.name,
        proj_to_copy_path=project_path,
...
def active_state_atsam_path() -> ActiveTestState:
    project_path = TEST_DATA_PATH / "test_projects/common_pwrforge_project_atsam"
    return ActiveTestState(
        target_id=pwrforgeTarget.atsam,
        proj_name=project_path.name,
        proj_to_copy_path=project_path,
...
@pytest.fixture
def test_state(request: FixtureRequest, tmp_path_factory: TempPathFactory, use_local_pwrforge: None) -> Any:
...
def setup_project(test_state: ActiveTestState) -> None:
    """Setup new project or copy existing project, chdir into it and run pwrforge update"""
...
class TestBinProjectFlow:
    ...
    def test_cli_build_profile_debug(self, test_state: ActiveTestState, setup_project: None) -> None:
        """Test pwrforge build --profile=Debug command is succesfull and bin file has correct
    ...
    def test_cli_build_profile_release(self, test_state: ActiveTestState, setup_project_build_release: None) -> None:
        """Test pwrforge build --profile=Release command is succesfull and bin file has correct
        format and compile_commands.json file contains correct cflags and cxxflags"""
    ...
    def test_cli_build_profile_new(self, test_state: ActiveTestState, setup_project_new_profile: str) -> None:
        """This test check if call of pwrforge build command for newly added profile will finish without error and
    ...
    def test_cli_run_new_proj(self, test_state: ActiveTestState, setup_project_build_release: None) -> None:
        """This test check if call of pwrforge run command will finish succesfully and with output 'Hello World!'"""
    ...
    def test_cli_check_and_fix_new_projects(
        ...
        """This test check if call of pwrforge fix command will finish without error and if no problems to fix
        will be found for newly created project"""
        if test_state.proj_to_copy_path:
            pytest.skip("This test is only for new project")

        if test_state.target_id == pwrforgeTarget.atsam:
    ...
    def test_cli_check_and_fix_copy_problematic_files(
        ...
        check if for copied files which contains some static code issues call of pwrforge check command will finish with
    ...
    def test_cli_test_with_gen_unit_test(
        ...
        """This test check if call of pwrforge test command will finish without error and if no tests were found
    ...
    def test_cli_gen_certs(self, test_state: ActiveTestState, setup_project: None) -> None:
        """Test pwrforge gen -c "device_id" command will finish without error and
    ...
    def test_cli_gen_filesystem(self, test_state: ActiveTestState, setup_project: None) -> None:
        """Test pwrforge gen -fs command will finish without error and if expected files under build/fs"""
        if test_state.target_id != pwrforgeTarget.esp32:
    ...
    def test_cli_doc(self, test_state: ActiveTestState, setup_project: None) -> None:
        """Test pwrforge doc command will finish without error and if documentation was generated"""
...
def test_project_x86_pwrforge_from_pypi(tmp_path: Path) -> None:
    # Test new and update work with pypi pwrforge version
    os.chdir(tmp_path)
    runner = pwrforgeTestRunner()

    with patch.object(_DockerComposeTemplate, "_set_up_package_version", return_value="pwrforge"):
