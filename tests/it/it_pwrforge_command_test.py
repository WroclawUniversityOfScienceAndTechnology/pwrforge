...
@dataclass
class pwrforgeCommandTestState:
    ...
    def __post_init__(self) -> None:
        ...
        if self.target_id == pwrforgeTarget.esp32:
    ...
    def set_config_src_extensions(self, ext: List[str]) -> None:
        config = get_pwrforge_config_or_exit()
        config_path = config.project_root / "pwrforge.toml"

        with open(config_path, encoding="utf-8") as pwrforge_config_file:
...
@pytest.fixture
def active_state_x86_path() -> pwrforgeCommandTestState:
    project_path = TEST_DATA_PATH / "test_projects/common_pwrforge_project"
    return pwrforgeCommandTestState(
...
@pytest.fixture
def active_state_esp32_path() -> pwrforgeCommandTestState:
...
@pytest.fixture
def active_state_atsam_path() -> pwrforgeCommandTestState:
...
@pytest.fixture
def setup_project(test_state: pwrforgeCommandTestState) -> None:
    """
...
@pytest.mark.xdist_group(name="TestFlowpwrforgeCommandTest")
class TestFlowpwrforgeCommandTest:
    ...
    def test_cli_basic(self, test_state: pwrforgeCommandTestState, setup_project: None) -> None:
        result = test_state.runner.invoke(cli, ["update"])
        assert result.exit_code == 0
    ...
    def test_cli_detailed_coverage(self, test_state: pwrforgeCommandTestState, setup_project: None) -> None:
        result = test_state.runner.invoke(cli, ["update"])
        assert result.exit_code == 0
    ...
    def test_cli_detailed_coverage_filtering(self, test_state: pwrforgeCommandTestState, setup_project: None) -> None:
